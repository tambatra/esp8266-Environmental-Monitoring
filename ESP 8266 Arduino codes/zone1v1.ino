...
void connect() {
  ...
  //Serial.print("\nConnecting...");
  while (!client.connect("zone 1", "try", "try")) {
    //Serial.print(".");
    digitalWrite(LED_SERVER, HIGH);
    delay(250);
    digitalWrite(LED_SERVER, LOW);
  ...
  client.subscribe("broker/connection1");
  client.subscribe("broker/calibration1");
  tickerBlink.attach_ms(500, blink);
  //Serial.println("\nconnected!");
  client.publish("iot/connection/zone1", "connected");
}

void messageReceived(String &topic, String &m_payload) {
  //Serial.println("incoming: " + topic + " - " + payload);
  String message = m_payload;
  if(topic == "broker/connection1"){
